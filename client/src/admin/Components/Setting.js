...
function Setting() {
  const[permissions , setPermissions] = useState({Student : false , Companies : false , Report : false , jobPosting : false , Resume : false})
   
  const[role , setRole] = useState("");

  const addPermission = async (role ,permission) => { 
    // API Call 
    const response = await fetch(`http://localhost:9002/admin/permission/addpermission`, {
      method: 'POST',
      headers: {
   ...
   const handleClick = (e)=>{
    ...
    setPermissions({Student : false , Companies : false , Report : false , jobPosting : false , Resume : false})
    setRole("");
    ...
    <div className="setting">
      <div>
        <div>
          <label>Role:</label>
          <br />
          <input type="text" placeholder="Enter Role" value = {role} className="input-aria"  onChange={handleOnChange} />
        </div>
