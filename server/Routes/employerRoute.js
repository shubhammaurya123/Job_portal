...
router.route('/api/login').post(async (req, res) => {
    ...
    const isPasswordValid = await bcrypt.compare(req.body.password, employer.password)
        ...
        await Employer.updateMany(
            ...
            },
            {
                $set: {
                    ...
                    companyName: (req.body.details.companyName == "") ? empl.companyName : req.body.details.companyName,
                    domainName: (req.body.details.domainName == "") ? empl.domainName : req.body.details.domainName,
                    designation: (req.body.details.designation == "") ? empl.designation : req.body.details.designation,
                    pinCode: (req.body.details.pinCode == "") ? empl.pinCode : req.body.details.pinCode,
                    address: (req.body.details.address == "") ? empl.address : req.body.details.address,
                }
            }
        )
...
router.route('/api/verifyOTP').post(async (req, res) => {
    ...
    if (!verifyAcc) return res.json({ status: 'error', error: 'Email Not Found!' })

    if (verifyAcc.OTP != req.body.OTP) return res.json({ status: 'error', error: 'Invalid OTP' })
        ...
        await Employer.updateOne({ email: req.body.email }, { $set: { verified: true } })
        return res.json({ status: 'ok' })
    }
})
...
router.route('/api/viewAllAdminJobs/:email').get(async (req, res) => {
    try {
        ...
        const posts = allPosts.filter((jobPost) => {
            return (
                jobPost.postedBy.email == req.params.email
            )
        })
        res.json(posts)
    }
